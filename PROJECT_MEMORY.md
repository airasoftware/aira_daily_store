...
- Pada 24 September 2026, deployment Vercel pengguna melaporkan `DATABASE_URL belum diatur` meskipun integrasi Supabase menyediakan nama env bawaan. Runtime kini menerima `DATABASE_URL` (prioritas), `POSTGRES_URL`, atau `POSTGRES_PRISMA_URL`; katalog menerima pasangan `NEXT_PUBLIC_SUPABASE_*` atau `SUPABASE_URL` + `SUPABASE_PUBLISHABLE_KEY`. Secret key Supabase tidak dipakai. Migrasi dapat memakai `POSTGRES_URL_NON_POOLING`. Tes jalur env lama dan integrasi Vercel (7 kasus) serta `npm run build` berhasil secara lokal. Koneksi pada deployment Vercel belum diverifikasi; deployment baru diperlukan agar kode terbaru berjalan.
