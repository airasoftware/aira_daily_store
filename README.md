...
Halaman **Keuangan** memakai pembukuan double-entry permanen dengan daftar akun, jurnal umum, buku besar, neraca saldo, neraca, dan laba rugi. Saat pesanan ditandai dibayar, sistem otomatis mendebit Kas, mengkredit Pendapatan Penjualan/Pengiriman, lalu mendebit HPP dan mengkredit Persediaan berdasarkan harga modal yang disnapshot ketika checkout. Isi harga modal produk/varian sebelum menerima pesanan; pesanan lama tanpa snapshot biaya tetap memiliki HPP nol dan perlu jurnal penyesuaian manual. Transaksi selain penjualan—modal, pembelian stok, beban, utang, pajak, serta koreksi—dicatat melalui jurnal manual yang hanya dapat disimpan jika total debit dan kredit sama. Perubahan jumlah stok dari editor produk tidak otomatis menebak sumber dananya, sehingga pembelian atau saldo awal persediaan juga harus dijurnal manual. Jurnal tidak dapat diedit/dihapus dari CMS; gunakan jurnal pembalik untuk koreksi agar jejak audit tetap utuh.
...
Deskripsi produk memakai editor WYSIWYG berbasis Tiptap di CMS (bukan TinyMCE). Toolbar menyediakan paragraf/judul/subjudul, tebal, miring, garis bawah, coret, daftar, kutipan, perataan, tautan, serta undo/redo. Pilih teks sebelum menambahkan tautan, lalu masukkan URL `https://` atau `http://`. Format tampil pada halaman detail produk. Deskripsi teks biasa yang sudah tersimpan tetap terbaca; HTML dari admin dibatasi ke elemen format yang didukung sebelum disimpan dan ditampilkan.
...
Sebelum merilis modul Keuangan, jalankan `npm run db:migrate`. Jika migrasi dilakukan melalui Supabase SQL Editor, gunakan [`db/20260926_finance_bookkeeping_supabase.sql`](db/20260926_finance_bookkeeping_supabase.sql). Skrip expand-only tersebut menambahkan harga modal, tanggal pembayaran, daftar akun, serta tabel jurnal; aman dijalankan ulang dan tidak menghapus data lama. Versi aplikasi sebelumnya tetap dapat dipakai tanpa menghapus kolom/tabel baru, sehingga rollback aplikasi tidak menghilangkan data pembukuan.
