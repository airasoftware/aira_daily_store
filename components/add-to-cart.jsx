...
export function AddToCart({ product, selectedColor = '', selectedSize = '', onColorChange = () => {}, onSizeChange = () => {} }) {
  ...
  const sizes = [...new Set(variants.filter(row => !selectedColor || row.color === selectedColor).map(row => row.size).filter(Boolean))];
  ...
  return <>
    ...
    {product.has_variants && <div className="variant-picker">{colors.length > 0 && <label>Warna<select value={selectedColor} onChange={event => { onColorChange(event.target.value); onSizeChange(''); setQuantity(1); setFeedback(''); }}><option value="">{variants.some(row => !row.color) ? 'Tanpa warna' : 'Pilih warna'}</option>{colors.map(value => <option key={value} value={value}>{value}</option>)}</select></label>}{(sizes.length > 0 || variants.some(row => row.size)) && <label>Ukuran<select value={selectedSize} onChange={event => { onSizeChange(event.target.value); setQuantity(1); setFeedback(''); }}><option value="">{variants.some(row => !row.size && (row.color || '') === selectedColor) ? 'All size' : 'Pilih ukuran'}</option>{sizes.map(value => <option key={value} value={value}>{value}</option>)}</select></label>}</div>}
    <p className="stock">{product.has_variants && !variant ? 'Pilih varian untuk melihat stok.' : stock > 0 ? `● Tersedia · ${stock} tersisa` : 'Stok habis'}</p>
