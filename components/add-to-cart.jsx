...
export function AddToCart({ product }) {
  ...
  const sizes = [...new Set(variants.filter(row => !color || row.color === color).map(row => row.size))];
  const variant = product.has_variants ? variants.find(row => row.color === color && row.size === size) : null;
  ...
  return <>
    ...
    {product.has_variants && <div className="variant-picker"><label>Warna<select value={color} onChange={event => { setColor(event.target.value); setSize(''); setQuantity(1); setFeedback(''); }}><option value="">Pilih warna</option>{colors.map(value => <option key={value} value={value}>{value}</option>)}</select></label><label>Ukuran<select value={size} onChange={event => { setSize(event.target.value); setQuantity(1); setFeedback(''); }} disabled={!color}><option value="">Pilih ukuran</option>{sizes.map(value => <option key={value} value={value}>{value}</option>)}</select></label></div>}
