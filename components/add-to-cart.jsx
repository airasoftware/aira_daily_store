...
export function AddToCart({ product, selectedColor = '', selectedSize = '', onColorChange = () => {}, onSizeChange = () => {} }) {
  ...
  const colors = [...new Set(variants.map(row => row.color).filter(Boolean))];
  const sizes = [...new Set(variants.map(row => row.size).filter(Boolean))];
  ...
  const unavailable = product.has_variants && !variant && Boolean(selectedColor && selectedSize);
  const outOfStock = stock < 1 && (!product.has_variants || Boolean(variant));
  const stockMessage = unavailable ? 'Kombinasi warna dan ukuran ini tidak tersedia.' : product.has_variants && !variant ? 'Pilih varian untuk melihat stok.' : stock > 0 ? `● Tersedia · ${stock} tersisa` : 'Stok habis untuk pilihan ini.';
  ...
  return <>
    ...
    {product.has_variants && <div className="variant-picker">{colors.length > 0 && <label>Warna<select value={selectedColor} onChange={event => { onColorChange(event.target.value); setQuantity(1); setFeedback(''); }}><option value="">{variants.some(row => !row.color) ? 'Tanpa warna' : 'Pilih warna'}</option>{colors.map(value => <option key={value} value={value}>{value}</option>)}</select></label>}{sizes.length > 0 && <label>Ukuran<select value={selectedSize} onChange={event => { onSizeChange(event.target.value); setQuantity(1); setFeedback(''); }}><option value="">{variants.some(row => !row.size && (row.color || '') === selectedColor) ? 'All size' : 'Pilih ukuran'}</option>{sizes.map(value => <option key={value} value={value}>{value}</option>)}</select></label>}</div>}
    <p className={`stock ${unavailable || outOfStock ? 'is-unavailable' : ''}`} role={unavailable || outOfStock ? 'alert' : undefined}>{stockMessage}</p>
