...
export function CartProvider({ children }) {
  ...
  function add(product, quantity = 1, variant = null) {
    setItems(current => {
      ...
      return [...current, { id: String(product.id), variantId: variant ? String(variant.id) : null, color: variant?.color ?? null, size: variant?.size ?? null, slug: product.slug, name: product.name, price: variant?.price ?? product.price, image_url: product.image_url, stock, quantity: Math.min(stock, quantity) }];
    });
  }
