...
export function parseProduct(body) {
  ...
  const parsedVariants = variants.map(row => ({ colorId: String(row?.colorId ?? ''), sizeId: String(row?.sizeId ?? ''), price: Number(row?.price), stock: Number(row?.stock) }));
  ...
  return { data: { name, slug, description, category, imageUrl, price, compareAtPrice: parsedVariants.length ? null : compareAtPrice, stock, variants: parsedVariants, badge: badge || null, featured: body.featured === true, newest: body.newest === true, active: body.active !== false } };
}
