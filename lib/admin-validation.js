...
export function parseProduct(body) {
  ...
  const parsedVariants = variants.map(row => ({ colorId: String(row?.colorId ?? ''), sizeId: String(row?.sizeId ?? ''), price: Number(row?.price), stock: Number(row?.stock), imageUrl: typeof row?.imageUrl === 'string' ? row.imageUrl.trim() || null : null }));
  if (parsedVariants.some(row => !/^\d+$/.test(row.colorId) || !/^\d+$/.test(row.sizeId) || !Number.isSafeInteger(row.price) || row.price < 0 || row.price > maxInteger || !Number.isSafeInteger(row.stock) || row.stock < 0 || row.stock > maxInteger) || new Set(parsedVariants.map(row => `${row.colorId}:${row.sizeId}`)).size !== parsedVariants.length) return { error: 'Kombinasi varian, harga, atau stok tidak valid.' };
  ...
  return { data: { name, description: productDescriptionHtml(description), category, imageUrl, price, compareAtPrice: parsedVariants.length ? null : compareAtPrice, stock, variants: parsedVariants, badge: badge || null, featured: body.featured === true, newest: body.newest === true, active: body.active !== false } };
}
