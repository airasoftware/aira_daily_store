...
export async function getProductVariants(client, productId, { includeInactive = false } = {}) {
  const { rows } = await client.query(`SELECT v.id, v.color_id, v.size_id, v.price, v.stock, v.is_active,
...
export async function saveProductVariants(client, productId, variants) {
  ...
  for (const variant of variants) {
    await client.query(`INSERT INTO product_variants (product_id, color_id, size_id, price, stock)
      VALUES ($1,$2,$3,$4,$5) ON CONFLICT (product_id, color_id, size_id)
      DO UPDATE SET price = EXCLUDED.price, stock = EXCLUDED.stock, is_active = TRUE`,
    [productId, variant.colorId, variant.sizeId, variant.price, variant.stock]);
  }
  if (variants.length) {
